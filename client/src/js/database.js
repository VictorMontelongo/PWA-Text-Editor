...
const initdb = async () =>
  openDB(payload, 1, {
    upgrade(db) {
      ...
      db.createObjectStore(payload, { keyPath: 'id', autoIncrement: true });
...
export const getDb = async () => {
  ...
  const jateDb = await openDB(payload, 1);
  const tran = jateDb.transaction(payload, 'readonly');
  const store = tran.objectStore(payload);
